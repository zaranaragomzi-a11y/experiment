# ==============================
# app.py — Streamlit Acid–Base Titration Lab (v2)
# ==============================
# 신규 기능
# 1) **지시약 추천 모드**: 등가점 pH를 기반으로 추천 TOP 지시약 리스트/적용 버튼 제공
# 2) **실험 CSV 업로드**: CSV의 (부피[mL], pH) 데이터를 읽어 시뮬 곡선과 오버레이, RMSE 계산
...
with st.expander("🔎 지시약 추천 모드 (등가점 기반)", expanded=True):
    if len(eq_pH) == 0:
        ...
        targets = eq_pH  # 여러 등가점이 있으면 모두 고려
        rows = []
...
with col1:
    ...
    if uploaded is not None:
        ...
        for key in cols:
            if 'vol' in key or 'mL'.lower() in key or '부피' in key:
                vol_col = cols[key]
            if 'ph' in key:
                ph_col = cols[key]
        # 기본 후보
